...
| 주행 상태 | 판단 및 동작 |
|---|---|
| 긴급 회피 | 장애물이 매우 가까우면 정지·후진 후 공간이 넓은 방향으로 회전 |
| 코너 회피 | 전방 공간이 부족하면 좌·우 거리를 비교해 회전 방향 결정 |
| 일반 주행 | 좌·우 거리 오차에 따라 양쪽 모터의 PWM 속도 보정 |
...
|---|---|
| `G` | 자율주행 즉시 시작 |
| `M` | 승객 탑승 확인 후 자율주행 시작 / 자율주행 중 수동 정지 전환 |
| `T` | 자율주행 중단 및 수동 정지 전환 |
| `F` / `B` | 수동 전진 / 후진 |
| `L` / `R` | 수동 좌회전 / 우회전 |
| `S` | 수동주행 정지 |
