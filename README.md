...
개발 환경: STM32CubeIDE, STM32 HAL, FreeRTOS
...
SensorTask가 좌·전방·우측 초음파 센서를 순차적으로 측정합니다.

측정값은 Mutex로 보호하여 DriveTask와 안전하게 공유합니다.
...
3방향 초음파 거리 측정 및 IR 센서 상태 확인

ManualTask

Above Normal

Bluetooth 명령 처리 및 자율·수동 모드 전환

DriveTask

Normal

장애물 회피 판단 및 차동 구동 기반 자율주행 제어
...
장애물이 매우 가까우면 정지·후진 후 공간이 넓은 방향으로 회전

코너 회피

전방 공간이 부족하면 좌·우 거리를 비교해 회전 방향 결정

일반 주행

좌·우 거리 오차에 따라 양쪽 모터의 PWM 속도 보정

일반 주행에서는 작은 거리 오차에 데드존을 적용해 불필요한 조향을 줄이고, 전방 장애물과 가까워질수록 기본 주행 속도를 낮추도록 구성했습니다.
...
승객 탑승 확인 후 자율주행 시작 / 자율주행 중 수동 정지 전환
...
Task와 센서 공유용 Mutex 생성

Sensor.c

초음파·IR 센서 측정 및 공유 데이터 관리

AutoDrive.c

자율주행 판단, 장애물 회피 및 P 제어

ManualControl.c

Bluetooth 명령 처리와 주행 모드 전환

Bluetooth.c

UART 인터럽트 기반 명령 수신 및 오류 복구
